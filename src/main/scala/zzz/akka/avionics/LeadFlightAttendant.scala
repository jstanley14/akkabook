...
class LeadFlightAttendant extends Actor { this: AttendantCreationPolicy =>
  ...
  override def preStart(): Unit = {
    ...
    val attendantNames = context.system.settings.config.
      getStringList("zzz.akka.avionics.flightcrew.attendantNames").asScala

    attendantNames take numberOfAttendants foreach { name =>
      context.actorOf(Props(createAttendant), name) }

//    val numberOfAttendants = context.system.settings.config.
//      getInt("zzz.akka.avionics.flightcrew.numberOfAttendants")
  }
